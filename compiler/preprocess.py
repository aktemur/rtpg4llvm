...
def getfraglist (text):
   ...
   global ccommentopen
   global ccommentclose
   global cppcommentopen
   cursor = 0
   txt = text
   ...
   ccommentopen = re.compile('/\*')
   ccommentclose = re.compile('\*/')
   cppcommentopen = re.compile('//')
   return fraglist(0)
...
def fraglist (s):
   ...
   while cursor < len(txt):
      ...
      if ob != None:
         ...
         cursor = nextrealchar(txt, cursor)
   return []
...
# nextrealchar(txt, cursor) returns the next character not
# contained in a quote or comment.  Specifically, if txt[cursor]
# is not the start of a string or comment, then this returns cursor+1.
# If it is, then it searches to the end of the comment and returns
# the cursor just after it.
def nextrealchar(txt, cursor):
   if txt[cursor] == '"':
      cursor = cursor+1
      while txt[cursor] != '"':
         if txt[cursor] == '\\':
         ...
         cursor = cursor+1
      return cursor+1
   elif cppcommentopen.match(txt, cursor) != None:
      cursor = cursor+2
      while txt[cursor] != '\n':
         cursor = cursor+1
      return cursor
   elif ccommentopen.match(txt, cursor) != None:
      cursor = cursor+2
      while ccommentclose.match(txt, cursor) != None:
         cursor = cursor+1
      return cursor+2
   else:
      return cursor+1
...
def splicecat (aqi):
   cat = aqi[0][4]
   code = aqi[1]
   if cat == "int" or cat == "double":
      code = "mkConst("+code+")"
      cat = "expr"
   if cat == "list":
      code = "mkConstList("+code+")"
      cat = "expr"
...
def process(file):
   codestr = readfile(file+'.cpp')

   prog = processprogram(codestr)

   fout = open(file + '-pp.cpp', 'w')
   fout.write(prog)
   return
